...
| 
SUBSTR(column \| expression, m[, n]) | m과 n 사이에 있는 문자열만 반환
(m = 음수 → 문자열의 끝에서 시작)
(n을 생략 → 문자열 끝까지 모두 반환) |
| LENGTH(column \| expression) | 표현식에서 문자의 갯수를 반환 |
| INSTR(column \| expression, ‘string’, [, m], [n]) | 문자열에서 string의 위치를 반환
(m → 탐색 시작점, n → n번째 탐색을 반환) |
| LPAD(column \| expression, n, ‘string’)
 RPAD(column \| expression, n, ‘string’) | n의 자릿수를 가진 문자열 형태로 오른쪽 정렬
n의 자릿수를 가진 문자열 형태로 왼쪽 정렬 |
| TRIM(leading \| trailing \| both trim_character FROM trim_source) | 문자열에서 첫 문자, 마지막 문자(또는 모두) 정리 |
| REPLACE(text, search_string, replacement_string) | 문자열에서 찾고자하는 text 탐색 
→ 존재 시, replacement_string으로 변환 |
...
| FUNCTION | PURPOSE |
| --- | --- |
| ROUND(column \| expression, n) | column, expression 또는 값에 대해 소수점 n번째 자리에서 반올림
(n 생략 → 소수점 표시 X) |
| TRUNC(column \| expression, n) | column, expression 또는 값에 대해 소수점 n번째 자리의 나머지는 버림
(n 생략 → n의 default value = 0) |
